...
fn draw_help_overlay(f: &mut Frame, app: &mut App) {
    ...
    let help_text = vec![
        ...
        Line::from(vec![
            Span::styled("  S           ", key),
            Span::raw("Cycle sort: Name↑ → Name↓ → ID↑ → ID↓ → Version↑ → Version↓ → off (Upgrades adds Available↑ → Available↓)"),
        ]),
        Line::raw(""),
        ...
        Line::from(vec![
            ...
            Span::raw("Sort by Name / ID / Version / Available (click again to reverse)"),
        ]),
        Line::from(vec![
