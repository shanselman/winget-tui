...
use crate::models::{OpResult, Operation, Package, PackageDetail, SourceFilter, SortDir, SortField};
...
mod tests {
    ...
    fn apply_filter_sort_by_name_asc() {
        ...
        app.apply_filter();
        let names: Vec<&str> = app.filtered_packages.iter().map(|p| p.name.as_str()).collect();
    ...
    fn apply_filter_sort_by_version_asc() {
        ...
        app.sort_dir = crate::models::SortDir::Asc;
        app.apply_filter();
        let versions: Vec<&str> = app.filtered_packages.iter().map(|p| p.version.as_str()).collect();
